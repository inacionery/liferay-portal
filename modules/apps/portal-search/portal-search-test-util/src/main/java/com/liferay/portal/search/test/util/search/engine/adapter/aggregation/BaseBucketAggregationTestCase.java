 ...
 * @author Inácio Nery
 */
public abstract class BaseBucketAggregationTestCase
	...
	public void testDateHistogramAggregationIntervalDay() throws Exception {
		...
		DateHistogramAggregation dateHistogramAggregation =
			new DateHistogramAggregation("date_histogram");

		dateHistogramAggregation.setField(Field.EXPIRATION_DATE);
		dateHistogramAggregation.setDateHistogramInterval("1d");
		dateHistogramAggregation.setMinDocCount(1L);

		AggregationResults aggregationResults = search(
			dateHistogramAggregation);

		DateHistogramAggregationResult dateHistogramAggregationResult =
			aggregationResults.getDateHistogramAggregationResult(
				"date_histogram");

		List<Bucket> buckets = dateHistogramAggregationResult.getBuckets();
	...
	protected void assertBucket(
		...
		Assert.assertEquals(expectedCount, bucket.getCount());
	}
